...
import { RemoveTagsOfChecked } from '@/infrastructure/repository/tag/RemoveTagsOfChecked';


export default function TagDeletePage () {
  // Const & View Model
  const { tagsInTodo, removeTagsInTodo, addDeleteTagIds, setIsChecked, getIsChecked, resetIsChecked } = useEditTodoViewModel()
  ...
  // While on this page.
  const RemoveTagsOfCheckedParams = {
    ...
    <main className='flex min-h-screen flex-col items-center justify-between p-24'>
      ...
      <div className='flex place-items-center'>
        ...
        <div className="mb-32 grid text-center lg:max-w-5xl lg:w-full lg:mb-0 lg:grid-cols-4 lg:text-left">
          <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-4 inline-block" onClick={() => RemoveTagsOfChecked(RemoveTagsOfCheckedParams)}>Remove Checked Tags</button>
          <Link href={"/todo_create"}><button type="button" className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-4 inline-block">戻る</button></Link>
        </div>
    </main>
  )
}