...
export const RemoveTagsOfCheckedInEditTodo = async ( params : {
  todoId: string, removeTagsInTodo: (tags: Array<Tag>) => void, removeTagIdsInTodo: (tagIds: string[], todoId:string) => void, getIsChecked: () => Tag[], addDeleteTagIds: (tagIds: Array<string>) => void, resetIsChecked: () => void, addToTagIds_for_add: (tagIds: string[]) => void, setAlertMessages: (alertMessages: string[]) => void, setShowAlert: (isShown: boolean) => void, resetAlertMessages: () => void,
}) => {
  const todoId = params.todoId;
  const removeTagsInTodo = params.removeTagsInTodo;
  const removeTagIdsInTodo = params.removeTagIdsInTodo;
  ...
  try {
    ...
    removeTagIdsInTodo(checkedTagIds, todoId);
